...
function soBuildAllocationMap_(oldDetails, newDetails) {
  ...
  const oldMap =
    soAggregateOutstandingAllocation_(oldDetails || []);

  const newMap =
    soAggregateOutstandingAllocation_(newDetails || []);

  console.log(
    JSON.stringify({
      diagnostic: "SO_ALLOCATION_RECONCILIATION",
      oldMap: oldMap,
      newMap: newMap
    })
  );
  ...
  Object.keys(itemIDs).forEach(itemID => {
    ...
    const oldQty =
      Number(oldMap[itemID]) || 0;

    const newQty =
      Number(newMap[itemID]) || 0;
  ...
  console.log(
    JSON.stringify({
      diagnostic: "SO_ALLOCATION_DELTA",
      deltaMap: deltaMap
    })
  );
